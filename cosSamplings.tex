...
\begin{algorithmic}
\Require{vectors $V$ ($|V|=n$); candidate pairs $P$ ($|P| = p$); features $U$; weights of the features $W = \{w_1,\cdots,w_{|U|}\}$; $\epsilon \in (0,1)$; $\delta \in (0,1)$.}
\Ensure{$Sim(u,v)$ for each $(u, v) \in P$.}
\State Introduce duplicate features based on the weights $W$, e.g., introduce $w_i-1$ duplicate features $i \in U$ if the feature $i$ has weight $w_i$.
\State Let $m$ be the number of features after the expansion.
\State $k \gets 0$;
